...
A lo largo de la práctica se construirán y estimarán modelos de precios hedónicos mediante estimaciones OLS. En último lugar, se relizará un pequeño ejercicio de clusters
...
Se incluye un apartado de caracter introductorio, en el que se realizará:
i) estadística descriptiva univariante y multivariante del dataframe house_train.
ii) análisis de correlaciones entre variables dependiente (price) e independiente y entre varaibles independientes pairwise
...
La variable 'date' parece reflejar la fecha de recogida de la observación. Se desconoce si es de utilidad, es decir, si está asociada a la fecha de fijación del precio (venta). Se crean las variables 'year' y 'month', para estudiar si hay variablidad y podría estar ligado a fecha de venta (o quizá de tasación). 
...
Gran parte de la información ya analizada puede visualizarse mediante boxplot. Asimimo, se puede detectar visualmente la presencia de outliers. En segundo lugar se incluirán histogramas para visualizar la forma de la distribución (ggplot+geom_histogram). En tercer lugar se incluirán scatterplots y matrices de correlación (ggairs).
...
Se han preseleccionado, hasta este punto, como variables determinantes de la superficie, sqfrt-living y sqft-lot. Se procede a abnalizar su relación.
...
Análisi gráfico multivariante: se procede al análisis gráfico conjunto de price y sqft-living. Los resultados se tendrán en cuenta a la hora de modelizar mediante OLS, ya que una de las exigencias del modelo es la linealidad.
...
Se concluye que el número óptimo de clusters teniendo en cuenta zipcode son 6. Si se ejecuta kmeans de nuevo con todas la variables, eliminando date, el número óptimo de cluster se sitúa en 4. Estas agrupaciones, son de gran importancia para la categorización de variables como zipcode que toma 70 registros. No se puede incluir en la regresión OLS una dummy con 70 registros ya que la ecuación estaría muy desbalanceada; y sabemos que zipcode tiene importancia en el precio.

Por ello, se va a proceder a agrupar zipcode en 5 grupos (no es la mejor técnica pero no nos funciona la insrucción de clsuter), según el percentil del price. 
...
Tabla de frecuecnias de la nueva varaible zip categorizada según percentiles y medianas de precios.
...
De estos dos ggpairs se concluye, que existe una relación positiva entre year-built y year-renovated y el número de bathrooms, no así con el número de bedrroms. ¿Refleja un cambio de preferencias a lo largo de los años? 
...
- Las variables relativas a las dependencias de una residencia tiene correlaciones positivas significativas con price: bathrooms y floors tienen una correlación alta del 0.5 y bedrooms del 0.3.  Estas tres varaibles tienen correlaciones altas entre sí (no bedrooms con floors) y con sqft-living
...
El principal inconveniente de la multicolinealidad consiste en que se incrementan la varianza de los coeficientes de regresión estimados hasta el punto que resulta prácticamente imposible establecer su significación estadística. Es decir, las elevadas varianzas hacen que los parámetros estimados sean imprecisos e inestables.
...
```{r}
drops2 <- c("date","sqft_living15","sqft_lot15", "month", "year", "renovated", "id", "bedroomscat", "bathroomscat", "floorscat", "conditioncat", "gradecat", "viewcat", "waterfrontcat", "sqft_basement", "logprice", "logsq_living", "sqft_above","zipcode", "catzip") 
# se eliminan aquells variables creadas en el exploratorio asi como las que determina la función alias
...
- Técnicas de reducción de la dimensión: se podrían aplicar técncas de reduciión de las dimensiones tipo PCA, eligiendo aquellos factores que expliquen un % de la varianza que se considere razonable. El problea de PCA es que las betas estimadas de los factores son de difícil interpretación. Es  decir, la estimación puede ser "buena" pero la interpretabilidad de los coefcientes prácticamente imposible.
 ...
 _ Otrastécnicas de selección de variables: podrían aplicarse técnicas de selección de variables como el algoritmo Boruta que está implementado en R. Como no se han repasado estas técnicas en clase, se adjunta un archivo separado de R en la que se aplica esta técnica. 

- Limpieza de outliers: En bienes que se adquieren pocas veces en la vida de un consumidor y que pueden tener para cada adquirente  un carácter único, individualizable e irrempazable -como es el caso de un residencia- debería contemplarse gran variabilidad y presencia de outliers. La limpieza de outliers - en todo caso a nivel multivariante con la distancia de Mahalanovis, por ejemplo- debe de realizarse con mucha precaución.  En este tipo de bienes pueden encontrase discontinuidades, es decir, mercados segmentados. Esto se analizará más adelante mediante estadísticos robustos.
...
Todos nuestros datos, tanto nuestras variables independientes así como nuestra variable dependiente, tienen que tener puntajes que están distribuidos normalmente. Más específicamente los residuos (error) de estos puntajes deben tener una distribución normal. La regresión es un análisis lineal y por ello, trabaja con relaciones lineales. Cuando los errores de las variables tienen distribución no normal, pueden afectar las relaciones y la significatividad. ¿Por qué se enfocan en los errores y no la medición en sí? Porque en una regresión lineal también es posible poner variables dicotómicas (sexo) y estas no tienen una distribución normal 
...
En este caso, parece que se cumplen los supuestos de normalidad y homogeneidad; auque existe una gran presencia de outliers. El R^2 es 0.48.
...
En esta primera fase de aplicación de OLS, con price como variable independiente y sqft_living como varaible independiente, la modelización que cumple con las asunciones para la aplicación de OLS y tiene un R^2 "aceptable"  es la especificación log-level, y en segundo lugar la log-log. 
...
- Nótese que no se han categorizado las variables bedrooms, bathrooms, floors, view, waterfront, grade; por lo que se entiende que hay ordinalidad y cardinalidad (es decir, que dos dormitoriso son el doble que 1, etc...). Su categorización provoca que el modelo esté desblanceado, se puede acabar con más de 100 dummies en el lado derecho
- Zipcode se ha reagrupado en una variable ordinal (catzip), y se ha categrozado en nivel (catzip2).
...
Inclusión de la variable de superficie con transformación logarítmica (para su poisble uso)
...
Parecen "cumplirse", aunque no de manera estricta, las hipotesis de normalidad y hommocedasticidad.
...
No hay diferencias considerables entre los estimadores OLS y estimadores OLS robustos. Solo hay diferencias en el estimador de bebrooms.

LLegados a este punto, tendríamos que señalar:

- el estimador de bedrooms es robutos (no converge)
- las beta estimadas son pequeñas en magnitud, en algunos casos (sqft_lot) cercanas a cero. Esto tiene que ver: el modelo no se incluye la variable depediente en nivel sino en logaritmo. El modelo también se ha estimado con las variables de superficie en logaritmos. Los residuos se siguen comportando bien; pero continuan el problema de estimador no robusto de bedrooms.
...
Llegados a este punto, se podría optra por comparar distintos modelos OLS con exclusión/inclsuín step-by-step de distintas combinaciones de bathrooms, bedrroms, sqft-living y grade o por modelos con coefcientes de regularización como Lasso o Ridge. Para dar mayor "varaibilidad ;D" al al ejercicio se opta por esta segunda opción.
